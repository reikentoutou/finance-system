---
name: Vue 财务统计全栈
overview: Vue + Nest 财务日報系统；**主交付**为前台机源码+终端+浏览器；**可选** Windows Electron 安装包。东京时区、一日四班次、真图、向导设密；**税务用导出**；**网管提交即入库无审批**；**管理员可查看/补录/改任意班次**；**导出仅管理员**；**23:59 截时、负偏差理由必填、底钱全局**；自动定期备份不要求，另见管理端 ZIP 备份。
todos:
  - id: init-monorepo
  ...
  - id: web-analytics
    ...
    status: pending
  - id: desktop-electron
    content: 备选：Electron Windows 安装包（内嵌 API+静态页，客户机须 Node）；主交付为源码+pnpm+浏览器
    status: pending
isProject: false
---
...
>
> **文档索引**：[docs/README.md](./README.md)（根目录 [README.md](../README.md)、[CHANGELOG.md](../CHANGELOG.md)、[RELEASING.md](../RELEASING.md)）。

# Vue + Node 财务统计系统实施计划（主交付：源码 + 终端 + 浏览器；桌面包备选）
...
```mermaid
flowchart LR
  browser[浏览器 / Vite 或静态站] -->|HTTP| nest[NestJS]
  nest --> db[(SQLite)]
  nest --> up[uploads]
  el[Electron 壳 可选] -.->|同机打包交付时| browser
```

## 技术选型（补充）

- 上传：`multipart/form-data`，Nest `FileInterceptor`；静态文件由 Nest 暴露 **`/uploads/`**（主交付下由浏览器直连 API；可选桌面包内由同机 Nest 提供）。
...
- **不做定期自动备份**（用户曾明确不要求自动跑）；若需防灾：现场可**手动拷贝** DB 与 `uploads`，或使用管理端 **「バックアップ・リストア」** 导出 ZIP（见 **[data-backup-restore.md](./data-backup-restore.md)**）。
- 磁盘占用与清理策略（旧图是否随日報删除）。
- PDF：**日文字体嵌入**与图片分辨率（须满足税务提交可读性）。

## 交付物

- **主交付**：本仓库源码 + 运行说明（根 **[README.md](../README.md)**）：前台机 **`pnpm install`**、配置 **`.env`**、**`pnpm run db:push`**、**`pnpm run dev`**（或 `build` + 常驻启动），浏览器访问；业务闭环含东京日 + 四班次唯一 + **网管提交无审批** + 真图 + 向导设密 + **导出仅管理员**。
- **备选**：Windows NSIS / 便携 **`.exe`**（Electron，客户机须 **Node**）；与主交付二选一或并存由实施方决定，见根 README「可选：Windows 桌面安装包」。
